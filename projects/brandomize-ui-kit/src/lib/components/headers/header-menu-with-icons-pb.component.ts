...
@Component({
  selector: 'bkit-header-menu-with-icons-pb',
  standalone: true,
      ...
      @if (mobileOpen()) {
        ...
        <aside [class]="getDrawerClasses()" role="dialog" aria-modal="true" aria-label="Navigation menu">
          ...
          <div class="flex-1 overflow-y-auto p-4">
            @for (link of getLinks(); track link.label) {
              <a [href]="link.url || '#'" [attr.target]="link.target" [class]="getMobileLinkClasses()" (click)="mobileOpen.set(false)">
                @if (link.icon) {
...
export class HeaderMenuWithIconsPbComponent {
