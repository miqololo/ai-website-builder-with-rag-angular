...
import { ComponentConfig } from '@brandomize/core/types/component-config.types';import { PButtonComponent } from '@brandomize/primitives/p-button/p-button.component';
import { ThemeService } from '@brandomize/core/theme/theme.service';

@Component({
  selector: 'bkit-header-with-cta-pb',
  standalone: true,
      ...
      @if (mobileMenuOpen()) {
        <div [class]="getMobileMenuClasses()">
          <div class="space-y-1">
            ...
            <a [href]="getCtaLink()" [class]="getMobileCtaClasses()" (click)="mobileMenuOpen.set(false)">
              {{ getCtaText() }}
            </a>
          </div>
        </div>
...
export class HeaderWithCtaPbComponent {
  ...
  getCtaText(): string {
    return (this.getData()['ctaText'] as string) || 'Get started';
  }
  ...
  getHeaderClasses(): string {
    ...
    return `sticky top-0 z-50 border-b border-${border} dark:border-${borderDark} dark:bg-gray-900/95`;
  }
  ...
  getMobileMenuButtonClasses(): string {
    ...
    return `rounded-md p-2 text-${navLink} hover:bg-${surfaceMuted} hover:text-${navLinkHover} dark:text-gray-300 dark:hover:bg-${surfaceMutedDark} md:hidden`;
  }
  ...
  getMobileLinkClasses(): string {
    ...
    const surfaceMuted = this.theme.getHeaderColor('surfaceMuted');
    const surfaceMutedDark = this.theme.getHeaderColor('surfaceMutedDark');
    return `block rounded-md px-3 py-2 text-base font-medium text-${navLink} hover:bg-${surfaceMuted} hover:text-${navLinkHover} dark:text-gray-300 dark:hover:bg-${surfaceMutedDark}`;
  }
  ...
  getMobileCtaClasses(): string {
    ...
    return `mt-4 block rounded-md bg-${primary} px-3 py-2 text-center text-base font-semibold text-white hover:bg-${primaryHover}`;
  }
