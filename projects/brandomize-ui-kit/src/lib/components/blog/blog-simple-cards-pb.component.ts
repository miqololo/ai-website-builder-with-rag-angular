...
@Component({
  selector: 'bkit-blog-simple-cards-pb',
  standalone: true,
  imports: [
    CommonModule,
    PSectionComponent,
...
export class BlogSimpleCardsPbComponent {
  ...
  getCardClasses(): string {
    ...
    return `block overflow-hidden rounded-2xl border border-${border} bg-${surface} shadow-sm transition-all duration-500 hover:-translate-y-2 hover:border-${primaryBase}-200 hover:shadow-xl dark:border-${borderMutedDark} dark:bg-${surfaceMutedDark} dark:hover:border-${primaryBase}-500/30`;
  }
  ...
  getTitleClasses(): string {
    ...
    const primary = t.colors?.primary ?? 'indigo-600';
    const primaryBase = primary.split('-')[0];
    return `mt-2 text-lg font-semibold text-${text} dark:text-white group-hover:text-${primary} dark:group-hover:text-${primaryBase}-400 line-clamp-2`;
  }
