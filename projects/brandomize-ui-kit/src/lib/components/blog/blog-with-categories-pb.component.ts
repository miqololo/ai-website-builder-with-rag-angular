...
@Component({
  selector: 'bkit-blog-with-categories-pb',
  standalone: true,
  imports: [
    CommonModule,
    PSectionComponent,
      ...
      <div class="mx-auto max-w-2xl text-center">
        ...
        <div class="mt-8 flex flex-wrap justify-center gap-2">
          @for (cat of getCategories(); track cat) {
            <button type="button" [class]="getCategoryButtonClasses()">{{ cat }}</button>
          }
        </div>
      </div>
      <div class="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-8 sm:mt-20 lg:max-w-none lg:grid-cols-2">
          ...
          <article class="group">
            <a [href]="post.link || '#'" [class]="getCardClasses()">
              <div class="relative aspect-[16/10] overflow-hidden">
                ...
                ></bkit-image>
                <span [class]="getCategoryBadgeClasses()">{{ post.category }}</span>
              </div>
              <div class="p-6">
...
export class BlogWithCategoriesPbComponent {
  ...
  getCategories(): string[] {
    return (this.getData()['categories'] as string[]) || ['Product', 'Design', 'Engineering', 'Marketing'];
  }
  ...
  getCategoryButtonClasses(): string {
    ...
    return `rounded-full bg-${primaryBase}-100 px-4 py-1.5 text-sm font-medium text-${primaryBase}-700 transition-colors hover:bg-${primaryBase}-200 dark:bg-${primaryBase}-500/20 dark:text-${primaryBase}-300 dark:hover:bg-${primaryBase}-500/30`;
  }
  ...
  getCardClasses(): string {
    ...
    const primary = t.colors?.primary ?? 'indigo-600';
    const primaryBase = primary.split('-')[0];
    return `block overflow-hidden rounded-2xl border border-${border} bg-${surface} transition-all duration-500 hover:-translate-y-1 hover:border-${primaryBase}-200 hover:shadow-lg dark:border-${borderMutedDark} dark:bg-${surfaceMutedDark} dark:hover:border-${primaryBase}-500/30`;
  }
  ...
  getCategoryBadgeClasses(): string {
    ...
    const surfaceMutedDark = t.colors?.surfaceMutedDark ?? 'gray-800';
    return `absolute top-4 left-4 rounded-full bg-${surface}/90 px-3 py-1 text-xs font-semibold text-${text} shadow-sm dark:bg-${surfaceMutedDark}/90 dark:text-white`;
  }
  ...
  getTitleClasses(): string {
    ...
    const primary = t.colors?.primary ?? 'indigo-600';
    const primaryBase = primary.split('-')[0];
    return `text-lg font-semibold text-${text} dark:text-white group-hover:text-${primary} dark:group-hover:text-${primaryBase}-400 line-clamp-2`;
  }
  ...
  getPosts(): Array<{ title: string; excerpt: string; image: string; date: string; author: string; category: string; link?: string }> {
    return (this.getData()['posts'] as Array<{ title: string; excerpt: string; image: string; date: string; author: string; category: string; link?: string }>) || [
      ...
      { title: 'Improve your customer experience', excerpt: 'Et tempora quod voluptatem nobis. Ut hic sit explicabo dolorum.', image: 'https://images.unsplash.com/photo-1547586696-ea22b4d4238d?w=800&h=500&fit=crop', date: 'Mar 10, 2024', author: 'Lindsay Walton', category: 'Design' },
      ...
      { title: 'Content strategy for growth', excerpt: 'Quaerat quasi aut velit incidunt excepturi rerum voluptatem minus harum.', image: 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&h=500&fit=crop', date: 'Mar 2, 2024', author: 'Tom Cook', category: 'Marketing' }
    ];
  }
}
