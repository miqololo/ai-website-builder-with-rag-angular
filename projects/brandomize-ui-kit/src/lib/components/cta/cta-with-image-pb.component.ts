...
@Component({
  selector: 'bkit-cta-with-image-pb',
  standalone: true,
  imports: [
    CommonModule,
    PSectionComponent,
          ...
          <div [class]="getImageContainerClasses()">
            <bkit-image
              [config]="{
                ...
                objectFit: 'cover',
                rounded: 'none',
                class: ['h-full w-full transition-transform duration-500 ease-out hover:scale-105']
              }"
            ></bkit-image>
          </div>
        </div>
...
export class CtaWithImagePbComponent {
  ...
  getImageContainerClasses(): string {
    ...
    return `relative aspect-video overflow-hidden rounded-2xl bg-${surfaceMuted} dark:bg-${surfaceMutedDark}`;
  }
  ...
  getImage(): string {
    return (this.getData()['image'] as string) || 'https://images.unsplash.com/photo-1521737711867-e3b97375f902?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1547&q=80';
  }

  getImageAlt(): string {
    return (this.getData()['imageAlt'] as string) || 'Call to action image';
  }
}
